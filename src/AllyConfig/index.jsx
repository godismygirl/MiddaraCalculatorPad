...
const AllyConfig = () => {
  const { activeAlly, allySave, updateAllySave, calculateDamage, damageDealt } =
    useGlobalStore();
  ...
  useEffect(() => {
    if (damageDealt.total > 0) {
      console.log("==== DMG Result ====", damageDealt);
      Modal.info({
        title: "Damage Dealt",
        content: (
          <div>
            <p>Total: {damageDealt.total}</p>
            <p>Physical: {damageDealt.physical}</p>
            <p>Magic: {damageDealt.magic}</p>
            <p>Irreducible: {damageDealt.irreducible}</p>
          </div>
        ),
      });
    }
          ...
          <Form form={form} layout="vertical">
            ...
            <Card title="FIELD STATUS">
              <Form.Item name="field_status" noStyle>
                <FieldStatus
                  userInput={userInput}
                  onUserInput={(u) => setUserInput({ ...userInput, ...u })}
                />
              </Form.Item>
            </Card>
