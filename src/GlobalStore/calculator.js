...
export const calculate = ({ enemy, preset, userInput }) => {
        ...
        if (condition) {
            ...
            switch (DMG_STEP_4[fd].dmg_type) {
                ...
                default: {
                    ...
                    if (
                        enemy.active_resist.includes('MAGIC') &&
                        !enemy.active_resist.includes('PHYSICAL')
                    ) {
                        paramCenter.physicalDamage += dmg;
                        return;
                    }
    ...
    //最后结算抗性
    if (enemy.active_resist.includes('PHYSICAL')) {
    ...
    if (enemy.active_resist.includes('MAGIC')) {
        if (paramCenter.magicDamage !== 1) {
            paramCenter.magicDamage = Number.parseInt(paramCenter.magicDamage);
        }
    }
