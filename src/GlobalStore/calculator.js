...
const alertMissed = ({ attack, defense }) => {
  Modal.error({
    title: "Attack Missed !",
    content: (
      <>
        <div className={css.row}>
          <div className={css.label}>Your Attack Roll</div>
          <div className={css.value}>
            <span className={css.errorStrong}>{attack}</span>
          </div>
        </div>
        <div className={css.row}>
          <div className={css.label}>Enemy Defense</div>
          <div className={css.value}>
            <span className={css.blackStrong}>{defense}</span>
          </div>
        </div>
      </>
    ),
...
const alertDamage = ({ total, physical, magic, irreducible }) => {
  Modal.success({
      ...
      <>
        ...
        <div className={css.section}>
          <div className={css.col}>
            <div className={css.label} style={{ background: "#16a085" }}>
              Physical
            </div>
            <div className={css.value}>
              <span className={css.strong} style={{ color: "#16a085" }}>
                {physical}
              </span>
            </div>
          </div>
          <div className={css.col}>
            <div className={css.label} style={{ background: "#ff4d4f" }}>
              Magic
            </div>
            <div className={css.value}>
              <span className={css.strong} style={{ color: "#ff4d4f" }}>
                {magic}
              </span>
            </div>
          </div>
          <div className={css.col}>
            <div className={css.label} style={{ background: "#f39c12" }}>
              Irreducible
            </div>
            <div className={css.value}>
              <span className={css.strong} style={{ color: "#f39c12" }}>
                {irreducible}
              </span>
            </div>
          </div>
...
export const calculate = ({ enemy, preset, userInput }) => {
  ...
  final_damage_step?.forEach((fd) => {
    ...
    if (DMG_STEP_4[fd].condition) {
      const params = DMG_STEP_4[fd].condition.params.map((p) => paramCenter[p]);
      ...
      switch (DMG_STEP_4[fd].dmg_type) {
        ...
        default: {
          ...
          if (condition.available.length <= 1) {
            ...
            if (t === "irreducible") paramCenter.irreducibleDamage += dmg;
            return;
          }
  ...
  if (enemy.activeResistance.includes("MAGIC")) {
    if (paramCenter.magicDamage !== 1) {
      paramCenter.magicDamage = Number.parseInt(paramCenter.magicDamage / 2);
    }
  }
